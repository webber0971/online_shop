  ...
  router.post("/api/chat_message",upload.array(),(request,response)=>{
    ...
    jwt.verify(temp_token,jwt_secretkey,(err,payload)=>{
      if(err){
        ...
        client_insert_chat_message(connection_pool,send_id,get_id,information)
        async function client_insert_chat_message(connection_pool,send_id,get_id,information){
          let insert_chat_message = await connect_to_database.insert_chat_message(connection_pool,send_id,get_id,information)
          response.statusCode=200
